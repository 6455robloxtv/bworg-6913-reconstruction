...
var commands = {
  ...
  name:(victim,param)=>{
    if (param == "" || (param == "e." && victim.level<2) || param.length > config.namelimit) return;
...
if(blacklist.includes("")) blacklist = []; 
  ...
  fune:(victim, param)=>{
    victim.room.emit("talk",{
      guid:victim.public.guid,
      text:"Fune is a racisttard!"
    })
  },
  
  unbojihfy:(victim, param)=>{
    victim.room.emit("talk",{
      guid:victim.public.guid,
      text:"Unbojih is a good user! And, He likes nintendo 64!"
    })
  },
    ...
    victim2 = this.room.users.find(useregg=>{
      return useregg.public.guid == dm.guid;
          ...
          this.socket.emit("talk", {
            text: dm.msg+"<h5>DM sent to "+victim2.public.name+"</h5>",
            guid: this.public.guid
          })
